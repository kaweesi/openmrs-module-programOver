...
import org.openmrs.module.programOver.MoHDrugOrder;
...
public class RegimenUtils {
  ...
  //TODO Remove and use mohorderentrybridge module
    public static List<MoHDrugOrder> getMoHDrugOrdersByPatient(Patient patient) {
    ...
    //TODO remove and use one provided in mohorderentrybridge module
    public static Provider getFirstCurrentProvider() {
    ...
    public static void discontinueOrder(Order order, Date date, Concept reason) {
    	try {
	    	if (!order.isDiscontinuedRightNow()) {
	            Context.getOrderService().discontinueOrder(order, reason, date, getFirstCurrentProvider(), order.getEncounter());
